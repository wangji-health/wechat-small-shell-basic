...
$('.rPost').on('click', function() {
	...
	var server = 'http://test.vongihealth.com'

	var getUrl = `${server}/wechat-web/wechat/invitationCode/bind/${$('.inputCode').val()}/${$('.inputNum').val()}`
		...
		success: function(data) {
			...
			if(data.status == 0) {
				...
				$('.word').on('click',function(){
					location.href = 'test.vongihealth.com/wechat-web/wechat/forwardPage/index'
				})
